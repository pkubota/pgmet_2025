...
MODULE Class_Module_Fields
 USE Constants, Only: r8, r4, i4, pi, Deg2Rad, r_earth, omega, nfprt
  ...
  INTEGER,PUBLIC, PARAMETER :: unitvmodel=63
  ...
  INTEGER                    :: lrec2D
  INTEGER                    :: lrec3D
  INTEGER,PUBLIC, PARAMETER  :: k_start=7
  INTEGER,PUBLIC, PARAMETER  :: k_finish=7
  ...
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: BETAPar (:,:) 
  ...
  REAL(KIND=r8),PUBLIC,ALLOCATABLE    :: z_med(:,:,:) 
  REAL(KIND=r8),PUBLIC,ALLOCATABLE    :: p_ref(:,:) 
  REAL(KIND=r8),PUBLIC,ALLOCATABLE    :: s_ref(:,:,:) 
  REAL(KIND=r8),PUBLIC,ALLOCATABLE    :: vo_ref(:,:,:) 
  ...
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: S_N(:,:,:) 
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: S_C(:,:,:) 

  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: Vort_N(:,:,:) 
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: Vort_C(:,:,:) 
...
CONTAINS
 ...
 SUBROUTINE Init_Class_Module_Fields() !INIT
  ...
  ALLOCATE(BETAPar(nLon,nLat));BETAPar=0.0
  ...
  ALLOCATE(Z_N(nLon,nLat,nLev));Q_N=0.0
  ALLOCATE(Z_C(nLon,nLat,nLev));Q_C=0.0

  ALLOCATE(S_N(nLon,nLat,nLev));S_N=0.0
  ALLOCATE(S_C(nLon,nLat,nLev));S_C=0.0

  ALLOCATE(Vort_N(nLon,nLat,nLev));Vort_N=0.0
  ALLOCATE(Vort_C(nLon,nLat,nLev));Vort_C=0.0
  ...
  ALLOCATE(z_med(nLon,nLat,nLev));z_med=0.0
  ALLOCATE(s_ref(nLon,nLat,nLev));s_ref=0.0
  ALLOCATE(vo_ref(nLon,nLat,nLev));vo_ref=0.0 
...
!  OPEN(unit=unitvmodel,FILE='../dataout/ModelV.bin',&
!      FORM='UNFORMATTED',ACCESS='DIRECT',RECL=lrec3D,ACTION='WRITE',STATUS='UNKNOWN') 
...
!!!!!!!!!!!!!!!!!!!!DEFINICAO DA MALHA (QUADRADA)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  DO j=1,nLat
     DO i=1,nLon
        BETAPar(i,j) = 2.0_r8*omega*cos(CoordLat(i,j))/r_earth
     END DO
  END DO
 ...
 SUBROUTINE ReadFields(irec,TimeIncrSeg) 
  ...
  DO k=1,nLev
     DO j=1,nLat
        DO i=1,nLon
           z_med(i   ,j,k) = sum(z_ref(:   ,:,k))/(nLat*nLon)
        END DO
     END DO
  END DO
  ...
  DO k=1,nLev
     DO j=1,nLat
        DO i=1,nLon
           !s_ref(i   ,j,k) = (((z_ref(i   ,j,k)-z_med(i   ,j,k))*9.81))/FcorPar(i,j)
           s_ref(i   ,j,k) = (((z_ref(i   ,j,k)-z_med(i   ,j,k))))/FcorPar(i,j)
...
!
 SUBROUTINE WriteFields(irec)
  ...
  WRITE(unitqmodel,rec=irec)REAL(S_C(:,:,k_start:k_finish),kind=r4) !SpecificHumidy
  WRITE(unitumodel,rec=irec)REAL(U_C(:,:,k_start:k_finish),kind=r4) !ZonalWind
  WRITE(unitvmodel,rec=irec)REAL(V_C(:,:,k_start:k_finish),kind=r4) !MeridionalWind
 ...
 SUBROUTINE Finalize_Class_Module_Fields() !FINALIZE
  ...
  CLOSE(unit=unitvmodel, STATUS='KEEP')
