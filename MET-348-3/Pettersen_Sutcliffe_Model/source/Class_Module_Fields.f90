...
MODULE Class_Module_Fields
 USE Constants, Only: r8, r4, i4, pi, Deg2Rad, r_earth, omega, nfprt
  ...
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: Z_N(:,:,:) 
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: Z_C(:,:,:) 

  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: W_N(:,:,:) 
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: W_C(:,:,:) 

  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: termA_QG(:, :)
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: termB_QG(:, :)
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: termC_QG(:, :)
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: termD_QG(:, :)
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: termQ_QG(:, :)
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: termS_QG(:, :)
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: total_forcing(:, :)  
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: thetas (:, :)
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: ug_surf(:, :)
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: vg_surf(:, :)
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: ug_iLND(:, :)
  REAL(KIND=r8),PUBLIC, ALLOCATABLE :: vg_iLND(:, :)
...
CONTAINS
 ...
 SUBROUTINE Init_Class_Module_Fields() !INIT
  IMPLICIT NONE
  REAL(KIND=r8), ALLOCATABLE :: pres_profile(:)
  INTEGER :: i,j
  ALLOCATE(Plevs(nLev));Plevs=0.0
  ALLOCATE(pres_profile(nLev));pres_profile=0.0
  ...
  ALLOCATE(Z_N(nLon,nLat,nLev));Z_N=0.0
  ALLOCATE(Z_C(nLon,nLat,nLev));Z_C=0.0

  ALLOCATE(W_N(nLon,nLat,nLev));W_N=0.0
  ALLOCATE(W_C(nLon,nLat,nLev));W_C=0.0

  ALLOCATE(termA_QG(nLon,nLat))
  ALLOCATE(termB_QG(nLon,nLat))
  ALLOCATE(termC_QG(nLon,nLat))
  ALLOCATE(termD_QG(nLon,nLat))
  ALLOCATE(termQ_QG(nLon,nLat))
  ALLOCATE(termS_QG(nLon,nLat))
  ALLOCATE(total_forcing(nLon,nLat))
  ALLOCATE(thetas (nLon,nLat))
  ALLOCATE(ug_surf(nLon,nLat))
  ALLOCATE(vg_surf(nLon,nLat))
  ALLOCATE(ug_iLND(nLon,nLat))
  ALLOCATE(vg_iLND  (nLon,nLat))  
...
!SAIDAS DO MODELO SAO SALVAS EM:
  OPEN(unit=unitumodel,FILE='../dataout/ModelSC.bin',&
      FORM='UNFORMATTED',ACCESS='DIRECT',RECL=lrec2D,ACTION='WRITE',STATUS='UNKNOWN')
      
  OPEN(unit=unittmodel,FILE='../dataout/ModelWD.bin',&
      FORM='UNFORMATTED',ACCESS='DIRECT',RECL=lrec2D,ACTION='WRITE',STATUS='UNKNOWN') 
      
  OPEN(unit=unitmmodel,FILE='../dataout/ModelFO.bin',&
      FORM='UNFORMATTED',ACCESS='DIRECT',RECL=lrec2D,ACTION='WRITE',STATUS='UNKNOWN') 
      
!  OPEN(unit=unitqmodel,FILE='ModelQ.bin',&
!      FORM='UNFORMATTED',ACCESS='DIRECT',RECL=lrec2D,ACTION='WRITE',STATUS='UNKNOWN')   
  OPEN(unit=unitqmodel,FILE='../dataout/ModelZG.bin',&
      FORM='UNFORMATTED',ACCESS='DIRECT',RECL=lrec2D,ACTION='WRITE',STATUS='UNKNOWN')   
...
!!!!!!!!!!!!!!!!!!!!DEFINICAO DA MALHA (QUADRADA)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  pres_profile(1:nLev)=abs(Plevs(1:nLev)-90000.0_r8)
  kMinLev=minloc(pres_profile,dim=1)
  deallocate(pres_profile)
 ...
 SUBROUTINE WriteFields(irec,irecwq,irecwf,ireczg)
  IMPLICIT NONE 
  INTEGER      , INTENT(INOUT) :: irec
  INTEGER      , INTENT(INOUT) :: irecwq
  INTEGER      , INTENT(INOUT) :: irecwf
  INTEGER      , INTENT(INOUT) :: ireczg
  irec=irec+1
  WRITE(unitumodel,rec=irec)REAL(termA_QG,kind=r4) !Temperature ,,,,termQ_QG,termS_QG,
  irec=irec+1
  WRITE(unitumodel,rec=irec)REAL(termB_QG,kind=r4) !Temperature ,,,,termQ_QG,termS_QG,
  irec=irec+1
  WRITE(unitumodel,rec=irec)REAL(termC_QG,kind=r4) !Temperature ,,,,termQ_QG,termS_QG,
  irec=irec+1
  WRITE(unitumodel,rec=irec)REAL(termD_QG,kind=r4) !Temperature ,,,,termQ_QG,termS_QG,

  irecwq=irecwq+1
  WRITE(unittmodel,rec=irecwq)REAL(thetas,kind=r4) !SpecificHumidy
  irecwq=irecwq+1
  WRITE(unittmodel,rec=irecwq)REAL(ug_surf,kind=r4) !SpecificHumidy
  irecwq=irecwq+1
  WRITE(unittmodel,rec=irecwq)REAL(vg_surf,kind=r4) !SpecificHumidy
  irecwq=irecwq+1
  WRITE(unittmodel,rec=irecwq)REAL(ug_iLND,kind=r4) !SpecificHumidy
  irecwq=irecwq+1
  WRITE(unittmodel,rec=irecwq)REAL(vg_iLND,kind=r4) !SpecificHumidy

  irecwf=irecwf+1
  WRITE(unitmmodel,rec=irecwf)REAL(total_forcing,kind=r4) !total_forcing
  ireczg=ireczg+1
  WRITE(unitqmodel,rec=ireczg)REAL(z_ref(:,:,16)-z_ref(:,:,1),kind=r4) !z_ref
