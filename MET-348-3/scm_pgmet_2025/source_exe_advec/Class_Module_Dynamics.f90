...
CONTAINS
 ...
 DO k=7,7
    ...
    END DO
 END DO
 PRINT*,it,MAXVAL(Q_C  (2:Idim,2:Jdim,7)), MINVAL(Q_C  (2:Idim,2:Jdim,7)),& !TESTAR
           MAXVAL(w_ref(2:Idim,2:Jdim,7)), MINVAL(w_ref(2:Idim,2:Jdim,7))
  ...
  DO k=7,7
      DO j=2,Jdim-1
         ...
         DO i=2,Idim-1
            ...
            uadvc = (1.0_r8/6.0_r8) * (u_ref(xf,yc,k) + u_ref(xc,yc,k) + u_ref(xc,yf,k) &
                                    +  u_ref(xc,yb,k) + u_ref(xc,yc,k) + u_ref(xb,yc,k))
            vadvc = (1.0_r8/6.0_r8) * (v_ref(xf,yc,k) + v_ref(xc,yc,k) + v_ref(xc,yf,k) &
                                    + v_ref(xc,yb,k) + v_ref(xc,yc,k) + v_ref(xb,yc,k))
