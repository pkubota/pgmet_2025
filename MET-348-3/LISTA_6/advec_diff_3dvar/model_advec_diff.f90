MODULE Class_Fields
 ...
 REAL (KIND=r8),PUBLIC :: DeltaT =0.1  
 REAL (KIND=r8),PUBLIC :: C =1.0
 REAL (KIND=r8),PUBLIC :: Kdif   = 2.0e-1_r8                        
 !
 !                                                                   1       1
 !                     'CFL=',C*DeltaT/DeltaX  <==> 0.2  ou 0.7  2 ----- *  ---
 !                                                                   4       1
 !
 INTEGER       , PUBLIC :: Idim
...
CONTAINS
!-----------------------------------------------------------------------------------------
  SUBROUTINE Init_Class_Fields(Idim_in)
    IMPLICIT NONE
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
MODULE ModAdvection
  USE Class_Fields, Only: DeltaT,DeltaX,Idim,r8,xa,tf0,tb0,yyf,xxb,xb0,xf0,Area,C,Kdif
   IMPLICIT NONE
   PRIVATE

  PUBLIC :: AnaliticFunction,RungeKutta6,RungeKutta4,RungeKutta3,RungeKutta2,Upstream

CONTAINS

!   
  FUNCTION AnaliticFunction(it)  RESULT (termX)
...
Module  Model
  ...
  USE ModAdvection, Only : AnaliticFunction,Upstream,RungeKutta6,RungeKutta4,RungeKutta3,RungeKutta2
...
CONTAINS
  ...
  SUBROUTINE Init(idim_in)
    integer       , intent(in) :: idim_in
      CALL Init_Class_Fields(idim_in)
      CALL InitClass_WritetoGrads()
  END SUBROUTINE Init

  ! ---------------- utility: identity ----------------
  function Run_Model(t,nn,tend,monotone) result(update)
